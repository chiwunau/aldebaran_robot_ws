...
#include <aldebaran_robot_ws/RectArray.h>
#include <aldebaran_robot_ws/Rect.h>

using namespace std;
using namespace ros;
using namespace cv;
//camera size
// SIZE_X=320 // must be modified to use camera_info
// SIZE_Y=240 
...
public:
  ...
  Learner_Class(): _it(n), detector(cv::FeatureDetector::create("SIFT")), extractor(cv::DescriptorExtractor::create("SIFT")), matcher(cv::DescriptorMatcher::create("BruteForce")){
    ...
    point_pub = n.advertise<aldebaran_robot_ws::RectArray>("/nao_learn", 10);
  ...
  void image_cb(const sensor_msgs::ImageConstPtr& msg_ptr){
    ...
    cv::rectangle(red_img,cv::Point(80, 60), cv::Point(240, 180), cv::Scalar(0, 0, 200), 3, 4);
    ...
    aldebaran_robot_ws::RectArray msg;
  ...
  void regist_cb(const std_msgs::String& st){ //will be changed to change size and position
    if(start_flag){
      ...
      cv::Mat cut_img(tmp_img, cv::Rect(80, 60, 160, 120));

      
      cv::Mat dst_img;
      cv::resize(cut_img, dst_img, cv::Size(), 0.5, 0.5);
      template_imgs.push_back(template_data(dst_img, detector, extractor, st.data.c_str()));
      //template_imgs.push_back(template_data(tmp_img, detector, extractor, st.data.c_str()));
